...
def fit_and_predict(estimator, data_train, label_train, data_train2, label_train2, line_number, dictionary):
  # fitting
  estimator.fit(data_train, label_train)

  # prediction
  label_predict = estimator.predict(data_train2)

  ary = [[0 for i in range(2)] for j in range(2)]
  correct = 0
  for i in range(0,len(data_train2)):
    ary[int(label_train2[i])-1][int(label_predict[i])-1] += 1
    diff = int(label_train2[i])  - int(label_predict[i])
    if diff == 0:
      correct += 1
  ...
  print >> sys.stderr, "accuracy: " + str(correct) + " / " + str(len(data_train2))

  #coef = estimator.get_coef()
  ...
  return [(float(correct) / len(data_train2)), factors]
  ...
  while True:
    for doc in tfidf_corpus:
      ...
      if ((i + offset) % (num_document / TEST )) == 0 and num_test < TEST and stars[int(labels[i])] < (TEST / NUM_STAR):
        data_train2.append(dense)
        label_train2.append(labels[i])
    ...
    if num_test == TEST:
      ...
      i = 0
      is_filling = True
  return [data_train, label_train, data_train2, label_train2, line_number]
...
## MAIN LOOP
  for i in range(0, TEST_COUNT):
    [data_train, label_train, data_train2, label_train2, line_target] = make_data(dictionary, tfidf_corpus, labels, line_ids, offset = i)

    estimator = LinearSVC(C=0.25);

    accuracy, factors = fit_and_predict(estimator, data_train, label_train, data_train2, label_train2, line_target, dictionary)
