...
$(function() {
    ...
    // This is the asynchronous process.
    ...
    */
    ...
    var parts = {"button-noun" : "名詞", "button-adjective" : "形容詞", "button-verb" : "動詞"};
...
function drawGraph(render_part) {
    ...
    for (var name in graph.data) {
        ...
        for (var depIndex in obj.depends) {
            ...
            //console.log(obj);
            link.stroke_width = obj.co_values[depIndex]; // by myself
            link.co2_terms = obj.co2_terms[depIndex]; // by myself
            link.co2_values = obj.co2_values[depIndex]; // by myself
    ...
    graph.colors = colorbrewer.Greens[config.graph.numColors];
    //hack
    graph.colors[2] = "#ff0000";
    ...
    graph.line = graph.svg.append('g').selectAll('.link')
      ...
      .enter().append('line')
        .attr('stroke-width', (function(d) { return Math.abs(d.stroke_width * 2000 + 4) + "px" })) // test
        .attr('stroke', (function(d) { return (d.stroke_width > 0) ? "#666" : "#e44" })) // test
        .attr('class', 'link')
        .on('mouseover', function(d) {
          d3.select(this).classed('hover-line', true);
          var x = d3.select(this)[0][0].x1.baseVal.value -200;
          var y = d3.select(this)[0][0].y1.baseVal.value -100;
          graph.svg.append('g')
          ...
          var svg = graph.svg.select('.lineGroup')
              svg
              .append('text')
              .text(function() {
                var s = d.source.name;
          ...
          for (var i = 0; i < d.co2_terms.length; i++){
            svg
              .append('text')
              .text(function() {
                return (d.co2_terms[i] + " : " +d.co2_values[i]);
              })
              .attr('x', x + 30)
              .attr('y', y + 40 + 20 *i )
        ...
        .on('mouseout', function() {
            graph.svg.select('.lineGroup').remove()
        });
    ...
    graph.node = graph.svg.selectAll('.node')
      ...
      .enter().append('g')
        ...
        .on('mouseout', function() {
            d3.select(this).select('.valueText').remove()
            if (!selected.obj) {
                if (graph.mouseoutTimeout) {
                    clearTimeout(graph.mouseoutTimeout);
                    graph.mouseoutTimeout = null;
    ...
    graph.nodeRect = graph.node.append('rect')
        .attr('rx', 40)
        .attr('ry', 40)
        .classed('positiveTerm', function(d){
            return d.value > 0;
        })
        .classed('negativeTerm', function(d){
            return d.value < 0;
        })
    ...
    graph.node.each(function(d) {
        ...
        lines.forEach(function(line) {
            var text = node.append('text')
                .text(line)
                .attr('font-size', function(d){return (Math.abs(d.value) *0.5 + 1 || 1 ) * 12 + 'px'})
                .attr('dy', dy + 'em');
            dy += ddy;
        });
    });
    ...
    setTimeout(function() {
        graph.node.each(function(d) {
            ...
            var value = (Math.abs(d.value) * 0.75 || 0) + 1;
            ...
            bounds.x1 -= padding.left * value;
            bounds.y1 -= padding.top * value;
            bounds.x2 += padding.left * value + padding.right * value;
            bounds.y2 += padding.top * value + padding.bottom * value;
